...
export class UrlStateService implements OnDestroy {
  ...
  // ============================================================================
  // LEGACY COMPATIBILITY: Maintain backward compatibility with old API
  // ============================================================================

  /**
   * @deprecated Use getQueryParam() instead
  ...
  /**
   * @deprecated Use getQueryParamSnapshot() instead
   * Legacy method for backward compatibility
   */
  getParam(paramName: string): string | null {
    return this.getQueryParamSnapshot(paramName);
  }

  /**
   * @deprecated Use setQueryParams() instead (returns Observable)
   * Legacy method for backward compatibility
   */
  updateParams(params: QueryParams, merge: boolean = true): void {
    if (merge) {
      this.setQueryParams(params).subscribe();
    } else {
      this.replaceQueryParams(params).subscribe();
    }
  }

  /**
   * @deprecated Use setQueryParams() with single key instead
   * Legacy method for backward compatibility
   */
  updateParam(paramName: string, value: string | null): void {
    const params: QueryParams = value !== null && value !== ''
      ? { [paramName]: value }
      : { [paramName]: undefined };  // undefined removes the parameter
    this.setQueryParams(params).subscribe();
  }

  /**
   * @deprecated Use clearQueryParam() instead
   * Legacy method for backward compatibility
   */
  ...
  /**
   * @deprecated Use clearAllQueryParams() instead
   * Legacy method for backward compatibility
   */
  clearAllParams(): void {
    this.clearAllQueryParams().subscribe();
  }
  ...
  /**
   * Legacy method for checking if parameter exists
   */
  hasParam(paramName: string): boolean {
    return paramName in this.queryParamsSubject.value;
  }
}
